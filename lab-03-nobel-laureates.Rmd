...
- *There are `r nrow(nobel)` observations of `r ncol(nobel)` variables in the `nobel` dataset. Each row of the dataset represents a single nobel laureate.*
...
- *This filter does indeed leave the dataframe `nobel_living` with `r nrow(nobel_living)` observations.*
...
```{r}
#| label: laureates_by_residence_country
nobel_living_science <- nobel_living %>%
  mutate(
    country_us = if_else(country == "USA", "USA", "Other")
  ) %>%
...
nobel_living_science %>%
  ggplot(aes(x = "", fill = country_us, colour = country_us)) + 
  geom_bar(position = "dodge") +
  facet_wrap(~ category, ncol = 1) +
  coord_flip() +
  labs(title = "Nobel laureates", subtitle = "By residence in USA at time of award", x = " ", colour = "Country of residence", fill = "Country of residence")
```

- *Looking at this graph, it does support the statement made by the buzzfeed article, even matching the rough proportions of the statistics in each category.*
...
```{r}
#| label: born_in_us_count
nobel_bornin_US <- nobel_living_science %>%
  mutate(
...
- *Of the winners who were living in the USA when they got their awards, `r nobel_bornin_US` were born in the USA*
...
nobel_living_science %>%
  ggplot(aes(x = born_country_us, fill = country_us, colour = country_us)) + 
  geom_bar() +
  facet_wrap(~ category) +
  coord_flip() +
  labs(title = "Nobel laureates", subtitle = "By whether they were born in the USA", x = "Country of residence", colour = "Country of birth", fill = "Country of birth")
```

- *This data does not actually support Buzzfeed's conclusion, and shows a much smaller proportion of US Nobel Laureates who were born in other countries, with none at all in the fields of Chemistry and Economics. Perhaps they combined the data from other parts of the chart into their second graph? Either way, these proportions are much smaller than the proportions that were shown in the article.*
...
- *Germany and the United Kingdom are tied for the highest amount of Nobel laureates living in the USA at 7 each, followed by China with 5.*
